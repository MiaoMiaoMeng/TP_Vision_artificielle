...
int MotionDetectionRefresh(EdIMAGE *image, EdIMAGE *imres, EdIMAGE *imref, unsigned char Seuil, float Oublie)
{
	...
	float ref_R, ref_V, ref_B;
	float iref_R, iref_V, iref_B;
			...
			POINT_X(point)++)
	{
		...
		ref_R = PIXEL_R(imref, point);
		ref_V = PIXEL_V(imref, point);
		ref_B = PIXEL_R(imref, point);
		iref_R = PIXEL_R(image, point);
		iref_V = PIXEL_V(image, point);
		iref_B = PIXEL_B(image, point);

		if ((dif_R < Seuil) && (dif_V < Seuil) && (dif_B < Seuil))
		{
			/* --- Pas de mouvement detecte --- */
			PIXEL_R(imref, point) = Oublie * iref_R + (1 - Oublie) * ref_R;
			PIXEL_V(imref, point) = Oublie * iref_V + (1 - Oublie) * ref_V;
			PIXEL_B(imref, point) = Oublie * iref_B + (1 - Oublie) * ref_B;
